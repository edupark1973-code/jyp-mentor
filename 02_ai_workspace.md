...
| **Step 6** | **심사위원단 검증** | **페르소나 29 (정부 창업 지원 사업 심사위원단 5인)**<br>- 이현우(수익), 김진태(기술), 박서연(HR), 최민준(마케팅), 정세진(확장) 위원의 깐깐한 모의 심사 보고서 발간 | Step 1 ~ 5 데이터 |
| **Step 7** | **최종 수정** | 심사위원단 코칭 및 멘토의 29대 검증 체크리스트 피드백 내용을 수렴하여 보완한 **'최종 합격 사업계획서'** 텍스트 빌드 | Step 1 ~ 6 데이터 + 멘토 피드백 |
...
### 💡 Step 6: 5인 가상 심사위원단 검증 프롬프트 (Persona 29)
- **System Prompt**:
  ```text
  너는 대한민국 정부 창업지원사업 심사위원단장(페르소나 29)이야. 너의 내부에는 이현우(수익성), 김진태(기술성), 박서연(조직력), 최민준(마케팅/GTM), 정세진(확장성) 위원이 소속되어 있어.
  그들에게 이전 1~5단계까지 작성된 사업계획서 초안을 넘기고, 각 위원별로 날카롭고 깐깐한 시선으로 현미경 평가 보고서를 내놓게 지시해줘.
  각 위원은 자비심을 배제하고 단점을 정확히 짚어내야 한다.
  ```
- **User Prompt**:
  ```text
  [이전 컨텍스트]
  ...
  [과업]
  위의 내용들을 기반으로 5인 가상 심사위원단의 현미경 개별 평가 및 코칭 리포트를 작성해줘.
  ```
...
- **편집 가능한 텍스트 에디터**: AI가 생성한 단계별 초안을 멘티가 직접 입맛에 맞게 커스텀 편집한 뒤 저장할 수 있는 Rich Text Editor(또는 단순 TextArea) 제공. (10-80-10 접근법 중 마지막 10%의 창업가 통찰 주입을 실현하는 기능)
...
- [ ] 6단계 심사위원단 검증이 완료된 결과물을 멘티가 깔끔한 한글(.hwp)이나 워드(.docx) 포맷용 텍스트 파일로 내보내기(Export)할 수 있는 파일 다운로드 링크가 제공되는가?
