...
반드시 정중하고 신뢰감 있는 개요체를 사용하며, 고객의 Pain Point를 절실하고 생생한 스토리텔링 방식으로 묘사하라.`,
    task: `초기 아이디어를 바탕으로 고객들이 겪는 표면적 불편함 3가지를 선별하고, 각 불편함에 근본 원인(Root Cause)을 도출하십시오.
...
객관적이고 명료한 개요로 보고서를 작성하라. 모든 가정은 측정 가능한 검증 지표와 연결하고 확인되지 않은 사실은 단정하지 말라.`,
...
전략적이고 냉철한 개요체를 사용하라. 경쟁 우위를 과장하지 말고 주장마다 검증 근거 또는 검증 계획을 제시하라.`,
    task: `목표 시장에서 경쟁사가 따라오기 어려운 차별성을 핵심 경쟁우위로 정의하라.
초기 틈새시장을 특정하고 20자 이내의 강력한 가치제안 슬로건을 제시하라.
직접 경쟁사, 간접 대안을 비교한 경쟁 지도를 만들고 초기 안착 이후 인접 시장(Adjacent Market)으로 확장할 3단계 성장 로드맵과 단계별 KPI를 작성하라.`,
  },
  5: {
