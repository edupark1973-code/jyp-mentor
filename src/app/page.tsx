        ...
        <header className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs font-black uppercase mt-1 tracking-[0.3em]">
    ...
    <div className="flex flex-col h-screen bg-slate-900 overflow-hidden text-slate-200">
      <header className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-900/50 backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-3 overflow-x-auto custom-scrollbar pb-1">
          ...
          <button onClick={() => { const link = `${window.location.origin}/?id=${lecture.id}`; navigator.clipboard.writeText(link); alert('초대 링크 복사 완료!'); }} className="px-5 py-2.5 bg-slate-700 text-white rounded-2xl font-black text-sm flex items-center gap-2 shadow-lg hover:bg-slate-600 transition-all"><LinkIcon size={18} /> 초대 링크 복사</button>
          <button onClick={() => window.open(`/lecture/live?id=${lecture.id}`, '_blank')} className="px-5 py-2.5 bg-indigo-600 text-white rounded-2xl font-black text-sm flex items-center gap-2 shadow-lg hover:bg-indigo-700 transition-all"><Maximize2 size={18} /> 라이브 모드</button>
          {role === 'admin' && (
            <button onClick={() => setIsAddingSection(true)} className="px-5 py-2.5 bg-pink-500 text-white rounded-2xl font-black text-sm flex items-center gap-2 shadow-lg hover:bg-pink-600 transition-all"><Plus size={18} /> 섹션 추가</button>
          )}
        </div>
      </header>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-6" onClick={() => setIsSettingsOpen(false)}>
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm text-slate-900" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-black mb-6 text-slate-900">강좌 설정 수정</h2>
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={allowStudentPosts} onChange={() => setAllowStudentPosts(!allowStudentPosts)} className="w-5 h-5 accent-indigo-600" /><span className="font-bold">수강생 업로드 허용</span></label>
            </div>
      ...
      <main className="flex-1 overflow-x-auto p-8 flex gap-8 items-start custom-scrollbar">
        ...
        {isAddingSection && (
          <div className="w-80 flex-shrink-0 bg-white/10 rounded-[2rem] p-5 border border-white/10 backdrop-blur-xl animate-in fade-in">
             <input autoFocus value={newSectionTitle} onChange={e => setNewSectionTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSection()} placeholder="섹션 제목..." className="w-full bg-white/10 border border-white/10 rounded-2xl px-5 py-3 text-white font-bold outline-none mb-4" />
             <div className="flex gap-3">
               <button onClick={addSection} className="flex-1 bg-pink-500 text-white py-3 rounded-xl font-black shadow-lg">생성</button>
               <button onClick={() => setIsAddingSection(false)} className="px-4 bg-white/5 text-slate-400 py-3 rounded-xl"><X/></button>
             </div>
          </div>
        )}
      </main>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-10 animate-in fade-in" onClick={() => setPreviewMedia(null)}>
            <div className="relative w-full max-w-5xl h-full flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
              <div className="absolute -top-12 right-0 flex items-center gap-4">
                <a href={previewMedia.url} target="_blank" rel="noopener noreferrer" download className="p-2 text-white/50 hover:text-white" onClick={e => e.stopPropagation()}><Download size={28} /></a>
                <button onClick={() => setPreviewMedia(null)} className="p-2 text-white/50 hover:text-white"><X size={32} /></button>
              </div>
              {previewMedia.type.startsWith('image/') ? (
                <img src={previewMedia.url} alt="Preview" className="max-w-full max-h-full object-contain rounded-xl shadow-2xl" />
              ) : <iframe src={`https://docs.google.com/gview?url=${encodeURIComponent(previewMedia.url)}&embedded=true`} className="w-full h-full rounded-2xl bg-white shadow-2xl border-none" />}
            </div>
        </div>
      )}
    </div>
...
function Section({ section, lecture, cards, role, onPreview }: any) {
  ...
  const handleFileUpload = async (e: any) => {
    ...
    try {
      ...
      const sRef = ref(storage, `board/${Date.now()}_${file.name}`);
  ...
  return (
    <div className="w-80 flex-shrink-0 flex flex-col max-h-full">
      <div className="flex justify-between items-center mb-6 px-3">
        ...
        {role === 'admin' && <button onClick={() => { if(confirm('삭제?')) deleteDoc(doc(db, 'sections', section.id)) }} className="text-slate-600 hover:text-red-400 transition-colors"><Trash2 size={18} /></button>}
      </div>
      <div className="flex-1 overflow-y-auto space-y-5 pr-3 custom-scrollbar text-slate-900 pb-20">
        {(role === 'admin' || (lecture.allowStudentPosts && user)) && !isAdding && (
          <button onClick={() => setIsAdding(true)} className="w-full py-5 border-2 border-dashed border-white/5 rounded-[2rem] flex items-center justify-center text-slate-600 hover:border-pink-500/50 hover:text-pink-500 hover:bg-pink-500/5 transition-all group"><Plus size={28} className="group-hover:rotate-90 transition-all duration-300" /></button>
        )}
        {isAdding && (
          <div className="bg-white rounded-[2rem] p-6 shadow-2xl relative animate-in zoom-in-95">
            ...
            <div className="flex justify-between items-center pt-4 border-t">
              <div className="flex gap-1">
                <button onClick={() => fileRef.current?.click()} className="p-2 text-slate-400 hover:bg-slate-50 rounded-xl transition-colors"><Paperclip size={20} /></button>
                <input ref={fileRef} type="file" className="hidden" onChange={handleFileUpload} />
                <button onClick={() => setShowLinkInput(!showLinkInput)} className="p-2 text-slate-400 hover:bg-slate-50 rounded-xl transition-colors"><LinkIcon size={20} /></button>
              </div>
              <div className="flex gap-2">
                 <button onClick={() => setIsAdding(false)} className="text-slate-300 font-bold px-2 hover:text-slate-500">취소</button>
            ...
            {showLinkInput && <input placeholder="https://..." value={linkUrl} onChange={e => setLinkUrl(e.target.value)} className="w-full mt-3 p-2 text-xs bg-blue-50 text-blue-600 rounded-lg font-bold outline-none" />}
          </div>
        )}
        {cards.map((card: any, index: number) => (
          <Card key={card.id} card={card} role={role} onPreview={onPreview} onMoveUp={index > 0 ? () => handleMoveCard(card.id, 'up') : null} onMoveDown={index < cards.length - 1 ? () => handleMoveCard(card.id, 'down') : null} />
        ))}
      </div>
    </div>
  );
...
function Card({ card, role, onPreview, onMoveUp, onMoveDown }: any) {
  ...
  const addComment = async (e: any) => {
    ...
    await addDoc(collection(db, 'comments'), { cardId: card.id, text: newComment, author: user?.displayName || '수강생', createdAt: serverTimestamp() });
    setNewComment('');
  };

  const togglePin = async () => { if (role === 'admin') await updateDoc(doc(db, 'cards', card.id), { isPinned: !card.isPinned }); };
  const handleCopy = async () => { await navigator.clipboard.writeText(`${card.title}\n${card.content}`); setIsCopied(true); setTimeout(() => setIsCopied(false), 2000); };
  const handleUpdate = async () => { if (role === 'admin') { await updateDoc(doc(db, 'cards', card.id), { title: editedTitle, content: editedContent, updatedAt: serverTimestamp() }); setIsEditing(false); } };

  return (
    <div className={`bg-white rounded-[2rem] shadow-sm group hover:-translate-y-1.5 transition-all border p-6 text-slate-900 mb-5 w-full overflow-hidden relative ${card.isPinned ? 'border-blue-500 ring-2 ring-blue-500/10' : 'border-slate-100'}`}>
           ...
           {role === 'admin' && (
             <div className="flex items-center gap-1 mr-2 border-r pr-2 border-slate-100">
               ...
               <div className="flex flex-col gap-0.5">
                 {onMoveUp && <button onClick={onMoveUp} className="text-slate-300 hover:text-blue-500 transition-colors"><ChevronUp size={14}/></button>}
                 {onMoveDown && <button onClick={onMoveDown} className="text-slate-300 hover:text-blue-500 transition-colors"><ChevronDown size={14}/></button>}
               </div>
             </div>
           )}
           <button onClick={handleCopy} className={`p-2 rounded-xl transition-all ${isCopied ? 'text-green-500' : 'text-slate-200 hover:text-slate-400'}`}>{isCopied ? <Check size={16}/> : <Copy size={16}/>}</button>
           {role === 'admin' && (isEditing ? <button onClick={handleUpdate} className="p-2 text-blue-500"><Check size={16}/></button> : <button onClick={() => setIsEditing(true)} className="p-2 text-slate-200 hover:text-blue-500"><Pencil size={16}/></button>)}
           {role === 'admin' && <button onClick={() => { if(confirm('삭제?')) deleteDoc(doc(db, 'cards', card.id)) }} className="text-slate-200 hover:text-red-500 transition-all"><Trash2 size={16}/></button>}
         </div>
      </div>
      {isEditing ? (
        <div className="space-y-2">
          <input value={editedTitle} onChange={e => setEditedTitle(e.target.value)} className="w-full font-black text-lg border-b outline-none focus:border-blue-500 text-slate-900" />
          <textarea value={editedContent} onChange={e => setEditedContent(e.target.value)} className="w-full text-xs font-bold border-b outline-none focus:border-blue-500 text-slate-600 h-24" />
        </div>
      ) : (
        <>
          {card.title && <h4 className="font-black text-lg mb-2 leading-tight text-slate-900 break-words">{card.title}</h4>}
          {card.content && <p className="text-[14px] text-slate-500 mb-4 whitespace-pre-wrap leading-relaxed font-bold text-slate-600 break-words">{card.content}</p>}
        </>
      )}
        ...
        {card.fileUrl && (card.fileType?.startsWith('image/') || card.fileType === 'application/pdf') && (
          <div onClick={() => onPreview({ url: card.fileUrl, type: card.fileType })} className="relative group/file cursor-pointer rounded-2xl overflow-hidden border border-slate-100 bg-slate-50/50">
            {card.fileType.startsWith('image/') ? (
              <img src={card.fileUrl} alt="Preview" className="w-full h-auto group-hover/file:opacity-90 transition-opacity" />
            ) : (
              <div className="flex flex-col items-center justify-center p-8 text-blue-500 hover:text-blue-600 transition-colors">
                ...
                <span className="mt-4 px-4 py-1.5 bg-blue-100 text-blue-700 rounded-full text-[10px] font-black uppercase shadow-sm">PDF Viewer</span>
              </div>
            )}
          </div>
        )}
      ...
      <div className="pt-5 border-t border-slate-50">
        ...
        <form onSubmit={addComment} className="relative mt-2">
           <input value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="댓글 작성..." className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-2.5 text-xs outline-none focus:border-indigo-300 transition-all text-slate-900 font-bold" />
