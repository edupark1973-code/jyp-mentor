    ...
    <div className={`bg-white rounded-[2rem] shadow-sm group hover:-translate-y-1.5 transition-all border p-6 text-slate-900 mb-5 w-full overflow-hidden relative ${card.isPinned ? 'border-blue-500 shadow-blue-100 ring-2 ring-blue-500/10' : 'border-slate-100'}`}>
      ...
      <div className="pt-5 border-t border-slate-50">
        ...
        <div className="space-y-3 mb-4 max-h-40 overflow-y-auto custom-scrollbar">
           {comments.map(c => <div key={c.id} className="flex items-start gap-2 text-[12px] bg-slate-50/50 p-3 rounded-2xl border border-slate-100/50 text-slate-700 font-bold"><div className="min-w-0 flex-1"><span className="font-black text-indigo-500 mr-2">{c.author}</span>{c.text}</div>{role === 'mentor' && <button type="button" onClick={() => void deleteComment(c.id)} className="shrink-0 rounded-lg p-1 text-slate-300 transition-colors hover:bg-red-50 hover:text-red-500" title="댓글 삭제" aria-label="댓글 삭제"><Trash2 size={13} /></button>}</div>)}
        </div>
